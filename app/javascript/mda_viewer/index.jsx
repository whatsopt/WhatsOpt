...
class MdaViewer extends React.Component {
  ...
  handleConnectionCreate(event) {
    ...
    const data = { from: this.state.filter.fr, to: this.state.filter.to, names };
  ...
  handleOpenmdaoImplChange(openmdaoImpl) {
    ...
    } else if (this.state.mda.impl.openmdao.components.use_scaling === openmdaoImpl.components.use_scaling) {
  ...
  render() {
    const errors = this.state.errors.map((message, i) => (<Error key={i} msg={message} onClose={() => this.handleErrorClose(i)} />));
    ...
    const xdsmViewer = (
      <XdsmViewer
        ref={(xdsmViewer) => this.xdsmViewer = xdsmViewer}
        api={this.api}
        ...
        <div>
          ...
          <div className="tab-content" id="myTabContent">
            ...
            <div className="tab-pane fade" id="disciplines" role="tabpanel" aria-labelledby="disciplines-tab">
              <DisciplinesEditor
                name={this.state.newDisciplineName}
                nodes={db.nodes}
    ...
    const { quality } = this.props.mda.impl.metamodel;
