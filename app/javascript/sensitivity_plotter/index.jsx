...
class SensitivityPlotter extends React.Component {
  ...
  componentDidMount() {
    ...
    api.analyseSensitivity(opeId, (response) => {
  ...
  render() {
    ...
    if (sensitivity) {
      ...
      if (saMethod === 'morris') {
        desc =
          'For each output, plot of standard deviation (\u03c3) vs mean (\u03bc*) of the effect of a given input (the closer to (0,0), the lesser influent the input is)';
        plots = outs.map((o) => (
          <MorrisScatterPlot key={o.outname} outVarName={o.outname} saData={o.sensitivity} />
        ));
      } else if (saMethod === 'sobol') {
        desc =
          'For each output, plot of Sobol first order (S1) and total order (ST) indices (share of the variance of the output that is due to a given input: the greater, the more influent the input is)';
        heatmaps = [
        ...
        plots = outs.map((o) => (
          <SobolScatterPlot key={o.outname} outVarName={o.outname} saData={o.sensitivity} />
        ));
      } else {
