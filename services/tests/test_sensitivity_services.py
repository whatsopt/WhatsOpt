...
class SensitivityAnalyserProxy(object):
    ...
    def compute_hsic(self, thresholding_type, xdoe, ydoe, quantile, g_threshold):
        return self._thrift_client.compute_hsic(
...
class TestSurrogateService(unittest.TestCase):
    ...
    def test_compute_hsic(self):
        ...
        hsic = self.analyser.compute_hsic(
            xdoe, ydoe, SensitivityAnalysisTypes.HsicThresholding.ZERO, 0.20, 0.0
        )

        np.testing.assert_allclose(
            hsic.r2,
            [0.0680324, 0.0683437, 0.0340282, 0.0102625, 0.0040782],
            atol=1e-5,
            rtol=1e-5,
        )
        np.testing.assert_allclose(
            hsic.indices,
            [0.00141214, 0.00141847, 0.000706194, 0.000212957, 8.46044e-05],
            atol=1e-5,
            rtol=1e-5,
        )
        np.testing.assert_allclose(
            hsic.pvperm, [0, 0, 0.00990099, 0.306931, 0.90099], atol=1e-5, rtol=1e-5
        )
        np.testing.assert_allclose(
            hsic.pvas,
            [0.000331762, 0.000508135, 0.00166787, 0.391596, 0.834844],
            atol=1e-5,
            rtol=1e-5,
        )
